...
def process_structure_core(  # pylint: disable=too-many-locals,too-many-statements,too-many-arguments
    ...
    qe_pw = (
        str(jinja2.escape(get_qe_pw(raw_code_dict, out_json_data)))
        .replace("\n", "<br>")
        .replace(" ", "&nbsp;")
    )
    qe_matdyn = (
        str(jinja2.escape(get_qe_matdyn(raw_code_dict, out_json_data)))
        .replace("\n", "<br>")
        .replace(" ", "&nbsp;")
    )
    cp2k = (
        str(jinja2.escape(get_cp2k(raw_code_dict)))
        .replace("\n", "<br>")
        .replace(" ", "&nbsp;")
    )
    crystal = (
        str(jinja2.escape(get_crystal(raw_code_dict)))
        .replace("\n", "<br>")
        .replace(" ", "&nbsp;")
    )
    vasp_gga = (
        str(jinja2.escape(get_vasp_gga(raw_code_dict)))
        .replace("\n", "<br>")
        .replace(" ", "&nbsp;")
    )
    vasp_gen = (
        str(jinja2.escape(get_vasp_gen(out_json_data)))
        .replace("\n", "<br>")
        .replace(" ", "&nbsp;")
    )
    ...
    return dict(
        ...
        time_reversal_note=(
            time_reversal_note if path_results["augmented_path"] else ""
        ),
...
def get_qe_pw(raw_data, out_json_data):
    ...
    lines.append("    ntyp = {}".format(len(set(raw_data["primitive_symbols"]))))
    lines.append("    <...>")
    lines.append("/")
    ...
    for s, p in zip(
        raw_data["primitive_symbols"], raw_data["primitive_positions_cartesian"]
    ):
        lines.append("{:4s} {:16.10f} {:16.10f} {:16.10f}".format(s, p[0], p[1], p[2]))
...
def get_crystal(raw_data):  # pylint: disable=too-many-locals
    ...
    lines.append(
        "{:d} {:d} 100 <...> <...> 1 0     !<...> - <...>: 1st band - last band".format(
            npath, shrinking_fac
        )
    )
    for i, path in enumerate(kpath_new):
        ...
        lines.append(
            "{:2d} {:2d} {:2d}  {:2d} {:2d} {:2d}    {:2s} -> {:2s}".format(
                c0[0], c0[1], c0[2], c1[0], c1[1], c1[2], klabel[i][0], klabel[i][1]
            )
        )
...
def get_vasp_gga(raw_data):
    ...
    for s in raw_data["path"]:
        ...
        c1 = raw_data["kpoints_rel"][s[1]]
        lines.append(
            "{:16.10f} {:16.10f} {:16.10f} 1    {:2s}".format(c0[0], c0[1], c0[2], s[0])
        )
        lines.append(
            "{:16.10f} {:16.10f} {:16.10f} 1    {:2s}".format(c1[0], c1[1], c1[2], s[1])
        )
        lines.append("\n")
...
def get_vasp_gen(out_json_data):
    ...
    nkpts = len(kplines)
    lines.append(
        "<...>  !Total number of k-points = {:2d} + No. of k-points from IBZKPT file".format(
            nkpts
        )
    )
    lines.append("reciprocal")
